...
@NamedQueries({
        ...
        @NamedQuery(name="Book.SearchByTitle", query="SELECT b FROM Book b JOIN Title t ON b.titleBarcode = t.titleBarcode WHERE t.title LIKE :criteria ORDER BY t.title, t.author"),
        @NamedQuery(name="Book.SearchByAuthor", query="SELECT b FROM Book b JOIN Title t ON b.titleBarcode = t.titleBarcode WHERE t.author LIKE :criteria ORDER BY t.author, t.title"),
