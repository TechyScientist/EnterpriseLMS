...
<% String pageName = "staff", pageTitle = "Add Copy Record"; %>
...
<%                  break;
                case SC_BAD_REQUEST: %>
                    That action must be done with the add copy form.
<%                  break;
            } %>
            </p>
            <audio src="assets/sound/bonk.mp3" style="display: none;" autoplay></audio>
<%      } else if(status == SC_CREATED) { %>
            <p id="success"><img src="assets/img/check.png" alt="Success"/><strong>Success</strong>: Copy created successfully.</p>
  ...
  <form action="AddCopyServlet" method="post">
      ...
      <div style="display: grid; grid-template-columns: min-content min-content; gap: 10px;">
        <div class="form-field" style="margin-top: 0;">
            <label for="barcode-type">Copy Barcode Type</label>
      ...
      <button type="submit" id="add-copy-submit" name="add-copy-submit">Create Copy Record<img src="assets/img/proceed.png" alt="Proceed"/></button>
  </form>
