    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(session.getAttribute("user") != null) {
            ...
            if(user.libraryStaff || user.libraryAdmin) {
                if(request.getParameter("add-title-submit") != null) {
                    String titleBarcode = request.getParameter("barcode"),
                            title = request.getParameter("title"),
                            authors = request.getParameter("authors");
                    boolean initialCopy = request.getParameter("add-copy").equals("Yes");
                    if(barcodeBean.isValidTitleBarcode(titleBarcode)) {
                        if(titleDao.findByBarcode(titleBarcode) == null) {
                            if(initialCopy) {
                                String copy = request.getParameter("copy-barcode").isEmpty() ?
                                        barcodeBean.generateCopyBarcode() : request.getParameter("copy-barcode");
                                if(barcodeBean.isValidCopyBarcode(copy)) {
                                    titleDao.create(new Title(titleBarcode, title, authors));
                                    bookDao.create(new Book(copy, titleBarcode));
                                } else {
                                    session.setAttribute("status", SC_NOT_ACCEPTABLE);
                                    response.sendRedirect("/library/add-title.jsp");
                                }
                            } else {
                                titleDao.create(new Title(titleBarcode, title, authors));
                            }
